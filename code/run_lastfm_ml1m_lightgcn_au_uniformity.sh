...
# Clean ordinary-LightGCN AU uniformity-weight sensitivity on LastFM/ML-1M.

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
output_root="${OUTPUT_ROOT:-/root/autodl-tmp/outputs/outputs_v5.0_lastfm_ml1m_au_uniformity}"
datasets="${DATASETS:-lastfm ml-1m}"
weights="${AU_UNIFORMITY_WEIGHTS:-0.1 0.5 1 2 5}"
...
python3 - "$weights" "$learning_rate" "$configured_decay" "$uniformity_t" <<'PY'
import math
import sys
weights = [float(raw) for raw in sys.argv[1].split()]
if not weights or len(weights) != len(set(weights)):
    raise SystemExit("Uniformity weights must be non-empty and unique")
if any(not math.isfinite(value) or value < 0.0 for value in weights):
    raise SystemExit("Uniformity weights must be finite and non-negative")
for label, raw, allow_zero in (
        ("TRAIN_LR", sys.argv[2], False),
        ("TRAIN_DECAY", sys.argv[3], True),
        ("AU_UNIFORMITY_T", sys.argv[4], False)):
    value = float(raw)
...
run_case() {
  local dataset="$1"
  local weight="$2"
  local case_root="${output_root%/}/${dataset}/au_uniformity_$(token "$weight")"
  local completed="$case_root/comparison_summary.json"
  if [[ "$dry_run" != "1" && "$skip_completed" == "1" && -f "$completed" ]]; then
    echo "Skip completed dataset=$dataset uniformity_weight=$weight"
    return
  fi
  ...
  echo "Start dataset=$dataset uniformity_weight=$weight"
  DATASET="$dataset" \
  ...
  echo "Done dataset=$dataset uniformity_weight=$weight"
}

total_count=$((${#dataset_values[@]} * ${#weight_values[@]}))
echo "LastFM/ML-1M LightGCN AU uniformity-weight sensitivity"
echo "  datasets:           $datasets"
echo "  uniformity weights:$weights"
echo "  alignment weight:   1"
echo "  uniformity t:       $uniformity_t"
...
echo "  CrossNorm/filter:   none/none"
...
analysis_command=(
  python3 "$script_dir/analyze_lastfm_ml1m_lightgcn_au_uniformity.py"
...
analysis_command+=(
  ...
  --output "$output_root/lastfm_ml1m_au_uniformity.json"
  --markdown "$output_root/lastfm_ml1m_au_uniformity.md"
)
"${analysis_command[@]}"

echo "AU uniformity sensitivity completed: $output_root"
echo "  table: $output_root/lastfm_ml1m_au_uniformity.md"
echo "  JSON:  $output_root/lastfm_ml1m_au_uniformity.json"
