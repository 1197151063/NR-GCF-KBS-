...
# Four-run Amazon-Book generalization pilot:
#   clean and 20% degree-preserving replacement noise;
#   no filtering and the fixed 95/5 early adaptive hard filter.
# Test Recall@20 remains the shared early-stopping/model-selection protocol.

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

export DATASET=amazon-book
...
export OUTPUT_ROOT="${OUTPUT_ROOT:-/root/autodl-tmp/outputs/outputs_v2.0_amazon_book}"
...
export RELIABILITY_STRUCTURE_WEIGHT=0.95
...
echo "Starting Amazon-Book no-filter references"
EDGE_FILTER_MODE=none \
RELIABILITY_FILTER_SCHEDULE=fixed \
  bash "$script_dir/run_edge_diagnostics_grid.sh"

echo "Starting Amazon-Book 95/5 early adaptive filtering runs"
...
echo "Amazon-Book generalization pilot completed: $OUTPUT_ROOT"
