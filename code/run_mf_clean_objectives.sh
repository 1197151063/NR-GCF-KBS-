...
# Clean MF baselines: BPR, SSM, and AU on all four paper datasets.
# Each run uses the same sampler, objective, evaluation, and best-epoch rule as
# its corresponding LightGCN baseline, with graph propagation disabled.

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
repo_root="$(cd "$script_dir/.." && pwd)"
profile_file="${PROFILE_FILE:-$repo_root/configs/mf_clean_objectives.json}"
datasets="${DATASETS:-lastfm ml-1m yelp2018 amazon-book}"
objectives="${OBJECTIVES:-bpr ssm au}"
seeds="${SEEDS:-2026}"
gpu_id="${GPU_ID:-0}"
output_root="${OUTPUT_ROOT:-/root/autodl-tmp/outputs/outputs_mf_clean_objectives}"
dry_run="${DRY_RUN:-0}"
...
values = [
    ...
    dataset_profile.get("objective_message_dropout", 0.0),
...
run_case() {
  ...
  local objective="$2"
  local seed="$3"
  local case_root="${output_root%/}/${objective}/${dataset}/seed_${seed}"
  local completed="$case_root/comparison_summary.json"
  if [[ "$dry_run" != "1" && "$skip_completed" == "1" && -f "$completed" ]]; then
    echo "Skip completed objective=$objective dataset=$dataset seed=$seed"
    return
  fi
  ...
  local epochs patience batch_size learning_rate init_method init_weight
  local decay ssm_tau message_dropout au_weight au_t
  IFS=$'\t' read -r epochs patience batch_size learning_rate init_method \
    init_weight decay ssm_tau message_dropout au_weight au_t \
    < <(profile_values "$objective" "$dataset")

  echo "Start MF objective=$objective dataset=$dataset seed=$seed"
  DATASET="$dataset" \
  NOISE_MODE=prepared \
  NOISE_RATIOS=0 \
  SEEDS="$seed" \
  ...
  echo "Done MF objective=$objective dataset=$dataset seed=$seed"
}

planned=$((${#dataset_values[@]} * ${#objective_values[@]} * ${#seed_values[@]}))
echo "MF clean objective experiment"
...
python3 "$script_dir/analyze_mf_clean_objectives.py" \
  --input "$output_root/all_runs.json" \
  --profile "$profile_file" \
  --output "$output_root/mf_clean_objectives_summary.json" \
  --markdown "$output_root/mf_clean_objectives_summary.md"

echo "MF clean experiment completed: $output_root"
echo "  table: $output_root/mf_clean_objectives_summary.md"
echo "  JSON:  $output_root/mf_clean_objectives_summary.json"
