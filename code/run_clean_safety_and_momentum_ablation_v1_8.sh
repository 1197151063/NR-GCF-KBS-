...
# Minimal follow-up to outputs_v1.7:
#   1) clean no-filter reference;
#   2) clean early structure-momentum filtering safety check;
#   3) noisy early filtering with structure-only ranking at the same
#      momentum-calibrated budget (structure_weight=1.0).
# The existing v1.7 noisy structure_weight=0.95 result is not rerun.
...
export OUTPUT_ROOT="${OUTPUT_ROOT:-/root/autodl-tmp/outputs/outputs_v1.8}"
...
echo "Starting clean no-filter reference"
NOISE_RATIOS=0 \
EDGE_FILTER_MODE=none \
RELIABILITY_FILTER_SCHEDULE=fixed \
RELIABILITY_STRUCTURE_WEIGHT=0.95 \
  bash "$script_dir/run_edge_diagnostics_grid.sh"

echo "Starting clean early fused-filter safety check"
run_early_filter 0 0.95

echo "Starting noisy same-budget structure-only ranking ablation"
run_early_filter 0.2 1.0
...
echo "Clean safety and momentum ablation completed: $OUTPUT_ROOT"
echo "Compare its 20%-noise structure_weight=1.0 run with outputs_v1.7's"
echo "20%-noise structure_weight=0.95 original_always run."
